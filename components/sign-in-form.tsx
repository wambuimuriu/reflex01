...
export function SignInForm() {
  ...
  return (
    <main className="auth-shell">
      ...
      <section className="auth-panel"><div className="auth-card"><div className="mobile-auth-brand brand"><div className="brand-mark"><Truck /></div><div><strong>reflex</strong><span>delivery operations</span></div></div><p className="eyebrow">Welcome back</p><h2>Sign in to Reflex</h2><p className="auth-subtitle">Use your workspace credentials to continue.</p><form onSubmit={submit} noValidate className="auth-form"><label htmlFor="email">Email address</label><Input id="email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.com" required /><label htmlFor="password">Password</label><div className="password-wrap"><Input id="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" required /><button type="button" className="password-toggle" onClick={() => setShowPassword((visible) => !visible)} aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff /> : <Eye />}</button></div>{error && <p className="auth-error" role="alert">{error}</p>}<button type="submit" disabled={pending} aria-busy={pending} className="auth-submit">{pending ? 'Signing in…' : 'Continue'}{!pending && <ArrowRight data-icon="inline-end" />}</button></form><div className="demo-note"><strong>Demo access</strong><span>1retailer@reflex.dev · Demo1234!</span><span>retailer2@reflex.dev · Demo1234!</span><span>dispatcher@reflex.dev · Demo1234!</span><span>rider1@reflex.dev · Demo1234!</span><span>rider2@reflex.dev · Demo1234!</span></div></div></section>
    </main>
  )
}
